...
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog'; // Add this import if you have a Dialog component
...
const messages = [
	{
		id: 1,
		senderId: 2,
		content: 'Hey! I saw your post about the lost textbook. Is it still missing?',
		timestamp: '10:30 AM',
		isCurrentUser: false
	},
	{
...
const positionBg = (position: string | null) => {
    if (!position) return "";
    if (position.toLowerCase().includes("president")) return "bg-blue-100 text-blue-800";
    if (position.toLowerCase().includes("rep")) return "bg-green-100 text-green-800";
    if (position.toLowerCase().includes("secretary")) return "bg-yellow-100 text-yellow-800";
								...
								<div className="flex items-center space-x-3">
									...
									<div>
										...
										<p className="text-sm text-muted-foreground">
											{selectedUser.online ? 'Active now' : 'Last seen 2 hours ago'}
										</p>
									</div>
								</div>
							</div>
								...
								filteredStudents.map(student => (
									<div
										...
										onClick={() => {
											...
											const existing = conversations.find(c => c.name === student.name);
											if (existing) {
												setSelectedConversation(existing.id);
											} else {
												alert(`Start conversation with ${student.name}`);
											}
										}}
									>
										...
										{student.position && (
											<span
												className={`ml-2 text-xs px-2 py-0.5 rounded font-semibold ${positionBg(student.position)}`}
											>
												{student.position}
											</span>
										)}
